...
// ---------------------
// Robust payment reservation (anti spam + anti collision)
// ---------------------
async function reservePaymentAmount(tgId) {
  ...
  // if still pending active -> keep same amount until expiry
  ...
  // atomic insert attempts (unique partial index prevents collisions)
  for (let i = 1; i <= PAY_SLOTS; i++) {
    const amount = (PAY_BASE + i * PAY_STEP).toFixed(6);
    try {
      const ins = await q(
        `insert into payments (tg_id, expected_amount, status, expires_at)
         values ($1, $2, 'pending', $3)
         returning *`,
        [tgId, amount, expiresAt]
      );
      return ins.rows[0];
    } catch (e) {
      if (e?.code === "23505") continue; // unique violation -> try next slot
      throw e;
    }
  }
...
// Join requests enforcement (si activé)
...
// ---------------------
// TronScan: normalization + logging + confirmation
// ---------------------
...
async function fetchTrc20TransfersLatest(addressBase58, limit = 60) {
  // TronScan endpoints can vary; this one is common for TRC20 transfers list.
  const url =
...
function normalizeTronScanTransfer(raw) {
  ...
  const fromAddr =
    raw?.from_address || raw?.fromAddress || raw?.from || raw?.sender || "";
  ...
  // Amount in sun if provided, else try amount_str
  let quantSun = null;

  if (raw?.quant != null) {
    try { quantSun = BigInt(raw.quant); } catch { quantSun = null; }
  } else if (raw?.amount_in_sun != null) {
    try { quantSun = BigInt(raw.amount_in_sun); } catch { quantSun = null; }
  } else if (raw?.amount_str != null) {
    try { quantSun = toSunUSDT(raw.amount_str); } catch { quantSun = null; }
  } else if (raw?.amount != null) {
    const s = String(raw.amount);
    try { quantSun = s.includes(".") ? toSunUSDT(s) : BigInt(s); } catch { quantSun = null; }
...
async function autoConfirmPaymentsV4() {
  ...
  // 3) confirm pending using logs (idempotent)
  for (const p of pending.rows) {
    const txHash = await tryConfirmPayment(p);
    if (!txHash) continue;

    // Only confirm if still pending (idempotent)
    await q(
      `update payments
...
// start watcher
setInterval(() => {
  autoConfirmPaymentsV4().catch((e) =>
    console.error("autoConfirmPaymentsV4:", e?.response?.data || e?.message || e)
  );
...
// ---------------------
// Admin rendering helpers + broadcast UI
// ---------------------
async function renderBroadcastHelp(ctx) {
  const text = `<b>📣 Broadcast</b>
